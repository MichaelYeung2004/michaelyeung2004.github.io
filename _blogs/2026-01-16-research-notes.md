...
论文的实验部分可以分为三个层次：
1.  **玩具实验 (Toy Experiments)**：在简单的一维和二维数据集上，直观地、可视化地展示CT/ACT的核心机制和优点。
2.  **消融研究 (Ablation Study)**：控制变量，深入探究ACT不同组成部分（如前向/后向成本、评判家）的作用。
3.  **真实图像实验 (Real-world Image Experiments)**：在像CIFAR-10这样复杂的真实图像数据集上，与当前主流的GAN模型（SOTA, State-of-the-art）进行性能对比。

#### 1. 玩具实验

**A. 一维正态分布实验**

![image-20260120161251180](https://michaelyeung2004.github.io/assets/images/image-20260120161251180.png)

这组图的实验设置是：
*   **真实分布** $\mu$：标准正态分布 $\mathcal{N}(0, 1)$，即均值为0，方差为1。
*   **生成分布** $\nu$：一个可学习的正态分布 $\mathcal{N}(\theta, e^\phi)$，其中我们要学习的参数是均值 $\theta$ 和方差的对数 $\phi$。
*   **目标**：通过优化，让生成分布和真实分布完全一样，也就是让 $\theta \to 0$ 且 $\phi \to 0$ (即方差 $e^\phi \to 1$)。
...
这张图展示了在训练过程中，各个关键数值是如何随着迭代次数（横坐标 `iteration`）的增加而变化的。

*   **红色曲线 ($\theta$)** 和 **蓝色曲线 ($e^\phi$)**：这是我们要学习的两个参数。可以看到，随着训练进行，$\theta$ 迅速从初始值下降到接近0，而 $e^\phi$ （代表方差）也从初始值下降到接近1。这说明我们的模型确实在朝着正确的目标学习。
*   **绿色曲线 ($W_2(\mu, \nu)^2$)**：这是**Wasserstein距离**。它作为一个参照物，也迅速下降到0，说明生成分布和真实分布越来越像。
*   **紫色/灰色/黄色曲线**：这三条是我们最关心的**CT成本**。
    *   **紫色 ($C(\mu \to \nu)$)**：前向成本。
    *   **灰色 ($C(\mu \leftarrow \nu)$)**：后向成本。
    *   **黄色 ($C(\mu, \nu)$)**：总CT成本（前向和后向的平均）。
*   **核心观察**：可以看到，所有的CT成本也都随着训练下降并趋近于0。一个有趣的现象是，CT成本的下降速度甚至比参数收敛更快，这说明CT是一个非常敏感的“尺子”，能够有效地指导优化。

**小结：第一张图告诉我们，使用CT作为损失函数，模型可以成功地学习到正确的参数，让生成分布匹配真实分布。**

---

#### **第二、三张图（中间）：梯度等高线图**

这两张图是理解**为什么**模型能成功学习的关键。它们展示了损失函数（CT成本）的**梯度场**。梯度就像地图上的坡度，指明了“下山最快”的方向。模型就是沿着梯度方向来更新参数的。

*   **坐标轴**：横坐标是参数 $\theta$，纵坐标是参数 $\phi$。
*   **背景色和等高线**：表示在该点 $(\theta, \phi)$ 处的**梯度大小**。颜色越深/等高线越密集，表示梯度越大，模型在该处更新的“动力”越足。
*   **红色的轨迹线**：这代表了实际训练过程中，参数 $(\theta, \phi)$ 的变化路径。

**第二张图 (关于 $\theta$ 的梯度 $\frac{\partial C}{\partial \theta}$)**
*   **观察**：可以看到，当 $\theta \ne 0$ 时，梯度值（由等高线数值-1.000, 0.000, 1.000等表示）很显著。梯度场像一个“山谷”，谷底在 $\theta=0$ 这条垂直线上（梯度为0）。红色的轨迹线正是沿着山坡（梯度）顺利地滑到了谷底。

**第三张图 (关于 $\phi$ 的梯度 $\frac{\partial C}{\partial \phi}$)**
*   **观察**：同样，梯度场也形成了一个山谷，谷底在 $\phi=0$ 这条水平线附近。红色轨迹线也成功地滑向了谷底。

**小结：这两张图从梯度的角度，可视化地证明了CT成本函数构建了一个良好的优化“地形”。这个地形没有出现梯度消失的“平原”，也没有让模型卡住的“局部陷阱”，而是形成了一个清晰指向最优解 $(\theta=0, \phi=0)$ 的“大碗”，使得基于梯度的优化可以顺利进行。**

---

#### **最后三张图（最右）：前向、后向、总成本的对比分析 (核心！)**

这三张图是**本文思想的精髓**所在。它们展示了当参数 $\phi$ 已经学习得比较好（接近最优，这里固定 $e^{-\phi}=5$ 即 $\phi$ 是一个负数）时，不同的CT成本是如何随着参数 $\theta$ 变化的。这揭示了前向和后向成本各自的“偏好”。

*   **横坐标**：参数 $\theta$。
*   **纵坐标**：各种CT成本。
*   **我们的目标**：找到让成本最小的那个 $\theta$ 值。理想情况下，最小值应该在 $\theta=0$ 处取得。

**第四张图 ($C(\mu \to \nu)$，前向成本)**
*   **观察**：这条曲线的**最低点在 $\theta$ 大于0的地方**。
*   **解读**：这意味着，如果只用前向成本来优化，模型会倾向于学习一个比真实分布更“胖”（方差更大）且均值略有偏差的分布。为什么？因为前向成本的目标是**覆盖 (Covering)** 所有真实样本。为了确保每一个真实样本（来自 $\mathcal{N}(0, 1)$）都能被低成本地“服务”到，生成一个更宽泛的分布是“最安全”的策略。这就是**模式覆盖**行为的体现。

**第五张图 ($C(\mu \leftarrow \nu)$，后向成本)**
*   **观察**：这条曲线的**最低点在 $\theta$ 远小于0的地方**。成本在 $\theta$ 较小的时候非常平坦，然后急剧上升。
*   **解读**：如果只用后向成本来优化，模型会倾向于学习一个均值偏差很大且可能非常窄的分布。后向成本的目标是确保**生成的每一个样本都是高质量的**（即落在真实分布的高密度区域）。为了达到这个目的，最简单的策略就是**模式崩塌 (Mode Collapse)**：只生成那些最有把握的、最靠近真实分布中心的样本。它不在乎是否覆盖了所有真实样本，只在乎自己生成的别出错。这就是**模式搜寻 (Mode Seeking)** 行为的体现。

**第六张图 ($C(\mu, \nu)$，总CT成本)**
*   **观察**：这条曲线是前两条曲线的完美结合。它的**最低点正好在 $\theta=0$ 附近**。
*   **解读**：前向成本像一个“左拉”的力，防止模型模式崩塌，鼓励它去探索和覆盖。后向成本像一个“右拉”的力，防止模型生成垃圾样本，鼓励它提升质量。当这两个力被**加权平均**后，它们达到了完美的**平衡**，最终将模型的参数引导到了正确的、不偏不倚的最优解上。

**B. 一维/二维混合高斯分布实验 (图2, 图3)**

*   **目标**：真实数据是混合在一起的几个高斯分布（比如两个或八个“沙堆”）。这是模拟**多模式 (multi-modal)** 数据的经典场景，也是测试模型是否会**模式崩塌**的“试金石”。
*   **图2 (训练过程)**：展示了ACT如何一步步学习一个双峰分布。一开始只学会了一个峰（模式崩塌），但随着导航器越来越准，它能“感知”到另一个被遗漏的模式区域成本很高，从而引导生成器去探索那个区域，最终两个峰都学会了。
*   **图3 (对比实验)**：这个实验非常关键。它比较了**ACT**和**Wasserstein距离 (WGAN)** 在不同**批量大小 (mini-batch size)** 下的性能。
    *   当批量很大时（N=5000），两者效果都很好。
    *   当批量变小时（N=200, N=20），WGAN的性能急剧下降，几乎学不到东西了。而**ACT的性能几乎不受影响**，依然非常出色。
    *   **为什么会这样？** WGAN的传输计划是为**每个小批量**“临时”计算的，小批量样本少，信息量不足，算出来的计划就非常不稳定。而ACT的**导航器是全局学习的**，它在所有见过的批次上不断更新和积累知识，所以它的“导航”能力不依赖于单个小批量的大小，表现得非常**鲁棒 (robust)**。这在计算资源有限时是一个巨大的优势。

#### 2. 消融实验

**A. 前向 vs. 后向的平衡**

![image-20260120154831798](https://michaelyeung2004.github.io/assets/images/photos/image-20260120154831798.png)
...
*   **发现**：
    ...
    *   当$\gamma=0$（纯后向）时，模型发生了严重的**模式崩塌**，只学会了其中一两个模式。
    *   当$\gamma$在0.25到0.75之间时，效果都很好，特别是$\gamma=0.5$时，既没有模式崩塌，样本质量也很高。
*   **结论**：这个实验再次证明了**双向平衡是ACT成功的关键**。同时，它也提供了一个可调参数 $\gamma$，让用户可以根据实际需求微调模型是更偏向“多样性”（覆盖）还是“高质量”（搜寻）。

**B. 对抗性ACT的验证**

*   **图6**显示，即使加入了对抗性的评判家 (Critic)，上述的平衡关系依然成立，并且$\gamma=0.5$仍然是最佳选择。这说明评判家的引入并没有破坏ACT原有的良好特性，而是增强了它。
