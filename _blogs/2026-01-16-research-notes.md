...
好的，我们现在进入论文的最后一部分：**实验与结论**。

理论说得再好，终究要靠实验来验证。这部分的目的就是通过一系列精心设计的实验，向读者展示ACT（摊销式条件传输）相比于现有方法，究竟好在哪里。
...
*   **目标**：这个实验最简单，真实分布是一个标准正态分布 $\mathcal{N}(0, 1)$，生成分布是 $\mathcal{N}(\theta, e^\phi)$。我们的目标是让模型学会 $\theta=0$ 和 $\phi=0$。
*   **核心发现 (右侧图)**：
    *   只用**前向成本**（蓝色曲线），成本的最低点在 $\theta > 0$ 的地方。这意味着模型倾向于生成一个比真实分布更“胖”的分布，以此来“**覆盖**”所有真实样本。这验证了前向成本促进**模式覆盖**的猜想。
    *   只用**后向成本**（橙色曲线），成本的最低点在 $\theta \to -\infty$ (即 $e^\theta \to 0$) 的地方。这意味着模型倾向于生成一个极窄的分布，只落在真实分布最中心的位置，以此来确保生成的每个样本都是“安全的”。这验证了后向成本促进**模式搜寻**的猜想，并可能导致**模式崩塌**。
    *   **前向+后向 (CT/ACT)**（绿色曲线），成本的最低点正好在最优解 $\theta=0$ 附近。它完美地平衡了前两者，既不至于覆盖过度，也不至于模式崩塌。

这个简单的实验非常有力地证明了**双向传输的必要性和有效性**。
...
**定性结果 (图7, 图8)**：展示了ACT生成的图像样本。可以看出，图像质量很高，多样性也很好，没有明显的模式崩塌迹象，这与定量的FID/IS指标结果相符。
